...
async def get_best_pin_image(product: Dict[str, Any], strategy: Dict[str, Any], pin_index: int, job_store: Any, job_id: str, used_urls: Set[str], agent: Any) -> Dict[str, Any]:
    ...
    # Recovery addition: exact Amazon ASIN searches precede title-based queries.
    composio: List[Dict[str, Any]] = []
    search_queries: List[str] = []
    if asin:
        search_queries.extend((f"{asin} Amazon product image", f"{asin} product photo", f"{asin} Amazon"))
    search_queries.extend((name, f"{name} product", query, f"{name} front view", f"{name} side view", f"{name} lifestyle", f"{name} detail"))
